...
type PostCreateFormProps = { categories: CategoryType[]; post: PostType };

const { updateDashboardPost } = PostService.Api;
...
export default function PostEditForm({
  ...
  let selectedCategories: string[] = post.categories.map((category) =>
  ...
  const [excerpt, setAbstract] = useState(post.excerpt);
  const [featured, setFeatured] = useState(post.featured);
  const [title, setTitle] = useState(post.title);

  async function attemptPostEdit(
    ...
    const request = await updateDashboardPost(post.slug, { params });
